...
resolves this layout for you. Finalist A reads unchanged; Finalist B always needs its two renames
...
space first: Finalist A is ~1 GB, Finalist B ~8 GB. To reproduce the layout on your machine:
...
**2. Run the official NMA loader notebook** for the finalist you need (open it in Colab — it runs and pulls its OSF tarballs), then **move** what it writes into `data/`, applying Finalist B's two renames. A and B write to *different* places (the "working dir" is Colab's `/content/…`, or the folder you launch the notebook from locally):

| Finalist | Loader (opens in Colab · pinned v3.0.2) | Where it writes | Move into `data/…` as |
|---|---|---|---|
| **A** | [`load_hcp_task_with_behaviour`](https://colab.research.google.com/github/NeuromatchAcademy/course-content/blob/v3.0.2/projects/fMRI/load_hcp_task_with_behaviour.ipynb) | `hcp_task/` (in the working dir) | `A_load_hcp_task_with_behaviour/hcp_task/` |
| **B** | [`load_hcp`](https://colab.research.google.com/github/NeuromatchAcademy/course-content/blob/v3.0.2/projects/fMRI/load_hcp.ipynb) | `DATA/hcp_task/`, `DATA/hcp_rest/`, `DATA/hcp/` (extracted into a **`DATA/` subfolder**) **plus** `atlas.npz` (in the working-dir root) | `B_load_hcp/` as `DATA/hcp_task/`→**`hcp_task_339/`**, `DATA/hcp_rest/`→`hcp_rest/`, `DATA/hcp/`→`hcp/`, `atlas.npz`→**`hcp_atlas_339.npz`** |
...
339-subject task archive B uses — **not** an additional dataset here; use `load_hcp` for Finalist B.
...
All `True` for the finalist you downloaded means the paths are in place — but path existence is **not** proof
...
For the narrated, side-by-side QC of both finalists see
...
Both finalists sit behind **one** interface in [`sandbox/jaime/`](../sandbox/jaime/) — `datasets.py` (I/O)
...
spec = ds.spec_b(DATA)                         # or ds.spec_a(DATA) — the A/B choice is still open
